...
person should consult ''[the use of MetaFont]({% post_url FAQ-useMF %}))''.
...
[''previewing documents with Type&nbsp;1 fonts'']({% post_url FAQ-PSpreview %})&nbsp;&mdash;
if even then.

More modern fonts may be used in place of the Computer Modern set.  The
[EC fonts]({% post_url FAQ-ECfonts %}) and the 
[Latin Modern fonts]({% post_url FAQ-uselmfonts %}) are both close relatives with
