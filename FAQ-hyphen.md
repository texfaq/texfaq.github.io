...
''[Accented words aren t hyphenated]({% post_url FAQ-hyphenaccents %})''.)
...
a [partial reinstallation]({% post_url FAQ-newlang %}) is necessary (note that 
[LuaTeX]({% post_url FAQ-luatex %}) relaxes this constraint).
...
`\hyphenation` is discussed in
  the context of
[hyphenation failure]({% post_url FAQ-nohyph %}).
