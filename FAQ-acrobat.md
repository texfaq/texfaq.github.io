...
generator such as [pdfTeX]({% post_url FAQ-pdftex %}).
...
-  process the document direct to PDF with pdfTeX,
    [LuaTeX]({% post_url FAQ-luatex %}), or [XeTeX]({% post_url FAQ-xetex %}).
...
[HyperTeX specification]({% post_url FAQ-hyper %}):
...
If you use Plain TeX, the [Eplain macros]({% post_url FAQ-eplain %}) can
...
[`dvips` output for distillation]({% post_url FAQ-dvips-pdf %})).
