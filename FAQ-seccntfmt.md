...
the question on
[the style of section headings]({% post_url FAQ-secthead %}).
...
The section number is typeset using the
  [LaTeX internal]({% post_url FAQ-atsigns %})
...
[LaTeX internal commands]({% post_url FAQ-atsigns %}), so the above
