...
[using underscores]({% post_url FAQ-underscore %}).  However, you must be
...
''[`\verb` in command arguments]({% post_url FAQ-verbwithin %})'').
...
(see ''[Active characters in command arguments]({% post_url FAQ-actinarg %})'').
